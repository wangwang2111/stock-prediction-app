...
def main():
    sidebar_option = st.sidebar.selectbox('Make a choice', ['Financial Dashboard','Visualize', 'Predict'])
...
def predict():
    ...
    model = st.radio('Choose a model', ['RandomForestRegressor', 'ExtraTreesRegressor', 'XGBoostRegressor', 'CatBoostRegressor'],index=3, horizontal=True)
...
def model_preprocess(df, num):
    ...
    TAI_converter_lag.create_lag_variables('time', ['close'], lag=num, num_of_days_to_lag=num_of_days_to_lag)
    # TAI_converter.create_lag_variables('time', ['roberta_neg', 'roberta_pos'], lag=1)
    df = TAI_converter_lag.get_data()
    
    df_w_lead = df.copy()
    df_w_lead['lead_close'] = df_w_lead['close'].shift(-num)
    df_w_lead['time'] = df_w_lead['time'].shift(-num)
    df_w_lead['isFuture'] = df_w_lead['isFuture'].shift(-num)
    
    df_features_select = df_w_lead[80:-num].drop(['SP500 return', 'return', 'vnindex return'],axis=1) 
...
def create_predicted_chart(df):
    ...
    fig = px.line(df, x=df.index, y=['lead_close', 'Predicted Close'], markers=True,
                  title=f'Comparison between predicted close and real close price {option}')

    # Update trace colors individually
    fig.update_traces(line_color='#ee7527', selector=dict(name='lead_close'))  # Primary color
...
def model_engine(model, num):
    ...
    global close_index
    close_index = df_available.drop(columns=[TARGET, 'isFuture']).columns.get_loc('close')
    ...
    rmse, r2, mae, mape = regression_score(y_test, preds)

    st.subheader("Regression metrics on the test set!")
    col1.metric(label="RMSE", value=round(rmse, 2), delta=f"{round(rmse - 0.02 * df_available.close.mean(), 2)}", delta_color="inverse")
    col2.metric(label="R-squared", value=f"{round(r2, 2)}%", delta=f"{round(r2 - 80, 2)}%")
    ...
    cm, accuracy, precision, recall, f1 = classification_score(x_test, preds, y_test)
    
    col1, col2, col3, col4 = st.columns([1,1,1,1])
    st.subheader("Classification metrics on the test set!")
    col1.metric(label="accuracy", value=f"{round(accuracy - 0.8, 2)*100}%", delta=f"{round(accuracy - 0.8, 2)}")
    col2.metric(label="precision", value=f"{round(precision - 0.8, 2)*100}%", delta=f"{round(precision - 0.8, 2)}")
    col3.metric(label="recall", value=f"{round(recall - 0.8, 2)*100}%", delta=f"{round(recall - 0.8, 2)}")
    col4.metric(label="f1-score", value=f"{round(f1 - 0.8, 2)*100}%", delta=f"{round(f1 - 0.8, 2)}%")
    # st.table(cm)
    
    prediction_actual_comparison_chart(df_available, model, x_train, y_train, y_test, preds)

    model.fit(X, y)
    y_new = model.predict(x_forecast)
    # st.session_state.model = model
    df_w_visualization = df_preprocessed[['lead_close', 'isFuture']].copy()
    ...
    # col1, col2, col3, col4 = st.columns([1,1,1,1])
    # rmse, r2, mae, mape = regression_score(y, y_pred_all)

    # st.subheader("Regression metrics on the entire set!")
    ...
    # cm, accuracy, precision, recall, f1 = classification_score(X, y_pred_all, y)
    
    # col1, col2, col3, col4 = st.columns([1,1,1,1])
    # st.subheader("Classification metrics on the entire set!")
    # col1.metric(label="accuracy", value=f"{round(accuracy - 0.8, 2)*100}%", delta=f"{round(accuracy - 0.8, 2)}")
    # col2.metric(label="precision", value=f"{round(precision - 0.8, 2)*100}%", delta=f"{round(precision - 0.8, 2)}")
    # col3.metric(label="recall", value=f"{round(recall - 0.8, 2)*100}%", delta=f"{round(recall - 0.8, 2)}")
    # col4.metric(label="f1-score", value=f"{round(f1 - 0.8, 2)*100}%", delta=f"{round(f1 - 0.8, 2)}%")
...
def prediction_actual_comparison_chart(df_available, model, x_train, y_train, y_test, y_pred):
    fig = go.Figure()
    ...
    fig.add_trace(go.Scatter(x=df_available.index[int(len(df_available)*0.8):], y=y_pred, mode='lines', name='Predicted Close (Testing)'))
...
def regression_score(actual, preds):
...
def classification_score(x, preds, y):
    prediction_comparison_test_set = np.concatenate((x[:,close_index].reshape(-1,1), preds.reshape(-1,1), y.reshape(-1,1)),1)

    # Convert the array to a DataFrame for easier manipulation
    df_comparison = pd.DataFrame(prediction_comparison_test_set, columns=['price_lag', 'price_pred', 'price_actual'])

    # Create a new column 'price_lag' with values lagged by 1 row
    # df_comparison['price_lag'] = df_comparison['price_actual'].shift(1)
    return_pred = ((df_comparison['price_pred']/df_comparison['price_lag']) - 1)*100
    return_actual = ((df_comparison['price_actual']/df_comparison['price_lag']) - 1)*100
    
    # st.write(return_pred)
    # st.write(return_actual)

    # Calculate squared differences
    squared_diff = (return_pred - return_actual) ** 2
    # Calculate mean squared error
    mse = np.mean(squared_diff)
    # Calculate RMSE
    rmse = np.sqrt(mse)

    class_y_pred = (return_pred > 0)
    class_y_actual = (return_actual > 0)
    cm = confusion_matrix(class_y_actual, class_y_pred)
    accuracy = accuracy_score(class_y_actual, class_y_pred)
    precision = precision_score(class_y_actual, class_y_pred)
    recall = recall_score(class_y_actual, class_y_pred)
    f1 = f1_score(class_y_actual, class_y_pred)
...
if __name__ == '__main__':
    main()
>>>>>>> acc0435072ee5b679861765d3cc733385ed4ff5b
