...
def get_working_days(start_date, end_date, holidays_list):
    ...
    # No need to check weekday here, because freq='B' already excludes weekends.
    working_days = all_dates[~all_dates.isin(holidays_list)]
...
def add_new_working_days(df, number_of_new_working_days):
    ...
    # Get date range based on existing data + new working days you want to add
    start_date = pd.to_datetime(df['time']).dt.date.min()
    latest_date = pd.to_datetime(df['time']).dt.date.max()

    # Estimate an end_date (this can be adjusted based on needs)
    # E.g., to add extra room for holidays in the year span
    end_date = pd.to_datetime(latest_date) + pd.DateOffset(days=365)

    # Get a list of years covered in the range
    years_needed = list(range(start_date.year, end_date.year + 1))

    # Get holidays for all those years
    vietnam_holidays = holidays.country_holidays(country_code, years=years_needed)

    vietnam_holidays.update({'2024-02-08': "New Year's Day"})

    # Generate all working days within the date range
    all_working_days = get_working_days(start_date, end_date, vietnam_holidays.keys())
    ...
    # Get the next working days (after latest_date)
    new_working_days = all_working_days[latest_date_index + 1: latest_date_index + 1 + number_of_new_working_days]

    # Create a new DataFrame for the new working days
    new_rows = pd.DataFrame({'time': new_working_days})
    new_rows['isFuture'] = True

    # Ensure 'isFuture' column exists in the original DataFrame and set to False
    ...
    return df, list(new_working_days)
